...
class TS207(TSBase):
    ...
    @property
    def desc(self) -> str:
        return "Create feature set(s) & Ingest from HTTP source (one step)"

    @property
    def long_desc(self):
        return ("Create feature set(s) & Ingest from HTTP source source (one step, without save and load featureset)")
